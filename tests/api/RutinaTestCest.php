...
class RutinaTestCest extends BaseApiTestBase
{
     ...
     * @param ApiTester $I
     */
    public function usercantListRutina(ApiTester $I)
    {
        $I->wantToTest(Role::ROLE_USER . ' no puede listar rutinas');
     ...
     * @throws \Exception
     */
    public function adminCantCreateDietas(ApiTester $I)
    {
        $I->wantToTest(Role::ROLE_USER . ' no puede crear rutinas');
        $this->auth($I, Role::ROLE_USER);
        $I->amBearerAuthenticated($this->token);
        $request = [
     ...
     * @throws \Exception
     */
    public function adminCanNotCreateRutinaPorValidacion(ApiTester $I)
    {
    ...
    public function cantcreateDiaRutina(ApiTester $I){

        $I->wantToTest(Role::ROLE_USER . ' no puede crear un dia de la rutina');
        $this->auth($I, Role::ROLE_USER);
        $I->amBearerAuthenticated($this->token);
        /** @var Rutina $rutina */
        $rutina = $I->grabEntitiesFromRepository(Rutina::class)[0];
        ...
        $I->sendPOST('api/rutinas/'.$rutina->getUuid().'/dias', json_encode($request));
        $I->seeResponseCodeIs(Response::HTTP_FORBIDDEN);
        $I->cantSeeInRepository(Dia::class, ['uuid' => $request['uuid']]);
    }
    ...
    public function cantcreateDiaEjercicio(ApiTester $I){

        $I->wantToTest(Role::ROLE_USER . '  no puede crear un dia ejercicio de la rutina');
        $this->auth($I, Role::ROLE_USER);
        $I->amBearerAuthenticated($this->token);
        /** @var Rutina $rutina */
        $rutina = $I->grabEntitiesFromRepository(Rutina::class)[0];
        ...
        $I->sendPOST('api/rutinas/'.$rutina->getUuid().'/dias/'.$dia->getUuid().'/ejercicios', json_encode($request));
        $I->seeResponseCodeIs(Response::HTTP_FORBIDDEN);
        $I->cantSeeInRepository(DiaEjercicio::class, ['uuid' => $request['uuid']]);
    }
